...
class Index {
    ...
    function toggleOwnFilter() {
        ...
        //update 'hide completed tasks filter'
        if (request('toggle') == 'pref_hide_completed_tasks') {
            //toggle database settings
            auth()->user()->pref_hide_completed_tasks = (auth()->user()->pref_hide_completed_tasks == 'yes') ? 'no' : 'yes';
            auth()->user()->save();
        }
        ...
        //set
        if (auth()->user()->pref_hide_completed_tasks == 'yes') {
            request()->merge(['filter_hide_completed_tasks' => 'yes']);
        }
