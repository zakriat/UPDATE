    ...
    public function search($id = '', $data = []) {
        ...
        if (in_array(request('sortorder'), array('desc', 'asc')) && request('orderby') != '') {
            ...
            if (Schema::hasColumn('subscriptions', request('orderby'))) {
                $subscriptions->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
            ...
            case 'subscription_id':
                $subscriptions->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('subscription_id', request('sortorder'));
                break;
            case 'plan':
                $subscriptions->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('subscription_gateway_product_name', request('sortorder'));
                break;
            case 'amount':
                $subscriptions->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('subscription_final_amount', request('sortorder'));
                break;
            case 'date_renewed':
                $subscriptions->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('subscription_date_renewed', request('sortorder'));
                break;
            case 'date_next':
                $subscriptions->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('subscription_date_next_renewal', request('sortorder'));
                break;
            case 'payments':
                $subscriptions->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('sum_payments', request('sortorder'));
                break;
            case 'status':
                $subscriptions->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('subscription_status', request('sortorder'));
                break;
            }
        } else {
            //default sorting
