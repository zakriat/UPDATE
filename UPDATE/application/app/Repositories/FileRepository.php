...
class FileRepository {
    ...
    public function search($id = '', $data = []) {
        ...
        if ($data['apply_filters']) {
            ...
            //filters: folder id
            if (request()->filled('filter_folderid') && is_numeric(request('filter_folderid'))) {
                $files->where('file_folderid', request('filter_folderid'));
            }
    ...
    public function processUpload($data = []) {
        ...
        if ($data["directory"] == '' || $data["filename"] == '') {
            Log::error("required information is missing (directory or filename)", ['process' => '[process-uploaded-file]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'data' => $data]);
            return false;
        }
        ...
        $new_file_path = BASE_DIR . "/storage/files/" . $data["directory"] . "/" . $data["filename"];

        //validation: file exists
        if (!file_exists($file_path)) {
            Log::error("the avatar file could not be found", ['process' => '[process-uploaded-file]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'path' => $file_path]);
            return false;
        }

        //check if the file is an image
        if (!is_array(getimagesize($file_path))) {
            Log::error("the avatar file is not an image", ['process' => '[process-uploaded-file]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'path' => $file_path]);
            return false;
        }
     ...
     * @return bool status outcome
     */
    public function processGeneralUpload($data = []) {
        ...
        //validation: file exists
        if (!file_exists($file_path)) {
            Log::error("the file could not be found", ['process' => '[process-uploaded-file]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'path' => $file_path]);
            return false;
        }
        ...
        //return file type (can be useful, rather thank returning 'true')
        $extension = pathinfo($new_file_path, PATHINFO_EXTENSION);
        if (is_array(@getimagesize($new_file_path)) && in_array($extension, ['jpg', 'png', 'gif'])) {
            return 'image';
        } else {
            return 'file';
        }
