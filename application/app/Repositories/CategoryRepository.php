...
class CategoryRepository {
     ...
     * @return object
     */
    public function getCategoryItems($type = '') {

        //new object
        $categories = $this->categories->newQuery();

        //only item categories
        $categories->where('category_type', 'item');

        //sort
        $categories->orderBy('category_name', 'asc');

        //eager load with sorting and filtering
        $categories->with([
            'items' => function ($query){
                $query->where('item_type', 'standard') 
    ...
    public function create() {
        ...
        $category->category_uniqueid = str_unique();
